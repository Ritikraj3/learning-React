...
export default function ExpenseTable({ expenses, setExpenses, setExpense, setEditingRowId }) {
  ...
  // use onChange={(e) => setCategory(e.target.value.toLocaleLowerCase())} To set category.
  //   map <tr> in <tbody> with filteredData . to get filtered category dynamically.

  const total = filteredResult.reduce((acc, curr) => acc + parseInt(curr.amount), 0);
  ...
  return (
    <>
      ...
      <table className="expense-table" onClick={() => 
        ...
        <tbody>
          {filteredResult.sort(sortCallback).map(({ id, title, category, amount }) => (
            <tr
              key={id}
              onContextMenu={(e) => {
                e.preventDefault();
                setMenuPosition({ left: e.clientX + 8, top: e.clientY + 4 });
                setRowId(id);
              }}
            >
              <td>{title}</td>
